...
void normalizeCommunity(vector<int> &l,vector<int> &dk,vector<int> &lk){
    map<int,int> mp;
    int cnt=0;
    for (int i=1;i<=N;i++){
...
void randomWalk(vector<int> &l,vector<int> &dk,vector<int> &lk,double k){
    vector<int> ranPop;
    for (int i=1;i<=pop;i++) ranPop.push_back(i);
    shuffle(ranPop.begin(),ranPop.end(),gen);
    vector<vector<int>> P;
    for (int i=0;i<=lenP-1;i++)
        P.push_back(whales[ranPop[i]].l);
    vector<int> randNode;
    for (int i=1;i<=N;i++) randNode.push_back(i);
...
void EP_WOCD(){
    ...
    for (int t=1;t<=T;t++){
        ...
        if (t%preriodn==0){
            for (int p=1;p<=pop;p++){
                if (totalMutation[p]){
                    double c=0.9;
                    cout<<successMutation[p]/totalMutation[p]<<endl;
    ...
    cout<<ans<<endl;
    normalizeCommunity(xBest, whales[1].dk, whales[1].lk);
    for (int i=1;i<=N;i++){
        cout<<xBest[i]<<" ";
    }
}
